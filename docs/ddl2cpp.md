...
| --path-to-custom-template | No | | Path to a file containing template for header generation, e.g. to include custom headers. |
...
You can use `--path-to-custom-template` to override the default templates. This might be useful to add custom includes or imports, for instance.
For reference, the default templates are shown here:
